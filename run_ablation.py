...
1. Ensures pseudo-labels are generated
2. Runs all 5 ablation configurations sequentially
3. Collects results and prints a comparison table

Usage:
  python run_ablation.py           # Full ablation (all 5 configs)
  python run_ablation.py --quick   # Quick test mode
  python run_ablation.py --configs A1_baseline A5_full_pipeline  # Subset
"""
...
def ensure_pseudo_labels(quick=False):
    """Generate pseudo-labels if they don't exist."""
    needed = any(
        not os.path.exists(os.path.join(PSEUDO_LABEL_DIR, f"train_{t}.json"))
        for t in ("mt5_base", "mt5_xlsum")
    )
    if not needed:
        print("Pseudo-labels already exist, skipping generation.\n")
        return True

    print("=" * 80)
    print("STEP 0: GENERATING PSEUDO-LABELS")
    print("=" * 80)
    cmd = [sys.executable, "generate_pseudo_labels.py"]
    if quick:
        cmd.append("--quick")
...
def run_experiment(config_name, quick=False):
    ...
    cmd = [sys.executable, "train_student_ablation.py", "--config", config_name]
    if quick:
        cmd.append("--quick")
    if incomplete_dir:
...
def find_latest_result(config_name, include_quick=False):
    ...
    include_quick=False returns only full runs (quick_mode != True).
    """
    if not os.path.exists(RESULTS_DIR):
        return None
...
def print_comparison_table(all_results):
    ...
    print("\n" + "=" * 100)
    print("ABLATION STUDY RESULTS — BanglaT5-small Student")
    print("=" * 100)
    ...
    # Teacher reference
    teacher_rl = 0.4058
    print(f"\nTeacher (BanglaT5) ROUGE-L: {teacher_rl:.4f}")
    if baseline_rl:
...
def save_combined_results(all_results):
    ...
    output = {
        ...
        "teacher_model": "csebuetnlp/BanglaT5 (fine-tuned)",
        "pseudo_label_teachers": ["google/mt5-base", "csebuetnlp/mT5_multilingual_XLSum"],
        "results": {},
    }
...
def main():
    ...
    print("=" * 80)
    print("ABLATION STUDY — Enhanced Knowledge Distillation")
    print("=" * 80)
