...
=====================================================================================
BanSum Dataset Version (bansum_lte_1000_tokens.json)

Uses BanSum-finetuned teacher models:
  - BanglaT5 (banglat5_bansum) for logit-level KD
  - mT5-base-bansum and mT5-xlsum-bansum for pseudo-label augmentation

Ablation Configurations:
  A1_baseline       : Fine-tune student alone (no distillation)
  A2_single_kd      : + Logit-level KD from BanglaT5-BanSum teacher (fixed temperature)
  A3_multi_teacher   : + Pseudo-labels from mT5-base-BanSum and mT5-XLSum-BanSum
  A4_adaptive_temp   : + Confidence-adaptive temperature scaling
  A5_full_pipeline   : + Intermediate encoder hidden state matching

Usage:
  python train_student_ablation_bansum.py --config A1_baseline
  python train_student_ablation_bansum.py --config A5_full_pipeline --quick
"""
...
# ============================================================================
# Ablation Configurations (same structure, different teachers)
# ============================================================================
...
ABLATION_CONFIGS = {
    ...
    "A2_single_kd": {
        "description": "Single-teacher logit-level KD (BanglaT5-BanSum, fixed temperature)",
        "use_logit_kd": True,
        "use_pseudo_labels": False,
        "use_adaptive_temp": False,
        "use_intermediate_matching": False,
    ...
    "A3_multi_teacher": {
        "description": "Multi-teacher: Logit KD + pseudo-label augmentation from mT5-BanSum",
        "use_logit_kd": True,
...
# ============================================================================
# Paths & Hyperparameters (BanSum-specific)
# ============================================================================

TEACHER_MODEL_PATH = "./banglat5_bansum_20260218_213532/final_model"
...
class EnhancedDistillationTrainer(Seq2SeqTrainer):
    ...
    def _adaptive_kd_loss(self, s_logits, t_logits, labels=None):
        ...
        token_ent = -(p_t * torch.log(p_t + 1e-10)).sum(dim=-1)
        ...
        kl = F.kl_div(log_s, p_t_scaled, reduction="none").sum(dim=-1)
    ...
    def evaluate(self, *args, **kwargs):
        """Clear GPU cache before eval to prevent OOM from beam search spike."""
        if self.teacher is not None:
            self.teacher.cpu()
...
# ============================================================================
# Data Loading & Preprocessing (BanSum version)
# ============================================================================

def load_bansum_splits():
    """Load BanSum JSON and split 80/10/10 (matching teacher training)."""
...
def load_and_prepare_data(tokenizer, config, quick=False):
    """Load BanSum data, optionally attach pseudo-labels, tokenize."""

    train_df, val_df, test_df = load_bansum_splits()

    # Cap validation to 2000 samples for speed
    if len(val_df) > 2000:
    ...
    cols = ["text", "summary"] + ([c for c in train_df.columns if c.endswith("_summary") and c != "summary"] if include_pseudo else [])
    train_ds = Dataset.from_pandas(train_df[cols].reset_index(drop=True))
    val_ds = Dataset.from_pandas(val_df[["text", "summary"]].reset_index(drop=True))
    test_ds = Dataset.from_pandas(test_df[["text", "summary"]].reset_index(drop=True))
...
def train_ablation(config_name, quick=False, resume_dir=None):
    """Train student model with specified ablation configuration on BanSum."""
    config = ABLATION_CONFIGS[config_name]

    print("\n" + "=" * 80)
    print(f"ABLATION (BANSUM): {config_name}")
    ...
    if config["use_logit_kd"] or config.get("use_intermediate_matching", False):
        ...
        print(f"  Teacher params: {tp:,} ({tp/1e6:.1f}M) [bf16]")

    # Student (gradient checkpointing enabled to reduce activation memory)
    ...
    print(f"  Student params: {sp:,} ({sp/1e6:.1f}M) [grad_ckpt ON]")
    ...
    student_model.generation_config.num_beams = 4
    ...
    # Output directory
    results_base = "ablation_results_bansum"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ablation study for enhanced KD (BanSum)")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
