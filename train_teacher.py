...
# BanglaT5 - Pre-trained T5 model for Bangla (BEST FOR BANGLA)
MODEL_NAME = "csebuetnlp/BanglaT5"
BATCH_SIZE = 16  # Suitable for BanglaT5
...
# ============================================================================
MAX_INPUT_LENGTH = 850  # Extended input for BanSum articles
MAX_TARGET_LENGTH = 256  # Extended output for longer summaries
...
def load_datasets():
    ...
    df = pd.DataFrame([{
        'text': item['main'],
        'summary': item['sum1']  # Using sum1 as the target summary
    } for item in data])
...
def train_model():
    ...
    print("\n" + "=" * 80)
    print("BanglaT5 MODEL FINE-TUNING - BANSUM DATASET")
    print("=" * 80)
    print(f"\nModel: {MODEL_NAME}")
    ...
    output_dir = f"banglat5_bansum_{timestamp}"
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        # Saving
        save_total_limit=False,  # Keep only 8 most recent checkpoints
    ...
    print("Pre-trained on: Bangla text corpus")
    ...
    print(f"\n🆕 Starting fresh BanglaT5 training")
