"""
Evaluate Ablation Models — BLEU, BERTScore, Semantic Similarity
================================================================
Generates summaries from each ablation best_model on the test set,
then computes:
  - BLEU (sacrebleu)
  - BERTScore (F1)
...
Usage:
  python evaluate_ablation_metrics.py
  python evaluate_ablation_metrics.py --max_samples 500   # faster subset
"""
...
TEACHER_DIR = "bangla_t5_teacher_finetuned_20251216_143715/final_model"
...
DESCRIPTIONS = {
    ...
    "A2_single_kd": "Single-teacher logit-level KD (BanglaT5, fixed temperature)",
    "A3_multi_teacher": "Multi-teacher: Logit KD + pseudo-label augmentation from mT5",
...
def compute_semantic_similarity(predictions, references, sem_model):
    ...
    # Pairwise cosine similarity (each pred vs its ref)
    similarities = []
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate ablation models with BLEU/BERTScore/Semantic Similarity")
    ...
    parser.add_argument("--include_teacher", action="store_true", help="Also evaluate the teacher model")
    ...
    # Load semantic similarity model once (shared across all evaluations)
    ...
    if args.include_teacher and os.path.exists(TEACHER_DIR):
        models_to_eval.append(("Teacher_BanglaT5", TEACHER_DIR, "Teacher: Fine-tuned BanglaT5"))
    ...
    for cfg_name, model_dir, desc in models_to_eval:
        ...
        # Filter out empty predictions for safety
    ...
    print(f"\n\n{'='*110}")
    print("ABLATION STUDY — BLEU / BERTScore / Semantic Similarity")
    print(f"{'='*110}")
    header = (
        f"{'Config':<24} {'BLEU':>7} {'BERT-P':>8} {'BERT-R':>8} {'BERT-F1':>8} "
    ...
    baseline_sem = None
    for cfg_name in CONFIGS + (["Teacher_BanglaT5"] if args.include_teacher else []):
        r = all_results.get(cfg_name)
        if r is None:
            print(f"{cfg_name:<24}  {'—':>7} {'—':>8} {'—':>8} {'—':>8} {'—':>8}  (not evaluated)")
            continue
        ...
        delta_sem = ""
        if cfg_name != "A1_baseline" and cfg_name != "Teacher_BanglaT5" and baseline_sem:
            diff = r["semantic_similarity"] - baseline_sem
            delta_sem = f" ({diff:+.4f})"

        print(
            f"{cfg_name:<24} {r['bleu']:>7.2f} {r['bertscore_precision']:>8.4f} "
    ...
    # Best config by semantic similarity
