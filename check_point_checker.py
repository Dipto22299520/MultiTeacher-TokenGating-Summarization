...
# Configuration
CHECKPOINT_DIR = "banglaT5_full_doc_20260215_123349"
...
INPUT_PREFIX = "summarize bangla news: "  # Training prefix used
...
print(f"   Evaluating on: {min(NUM_TEST_SAMPLES, len(test_data))} samples")

# Sample test data
import random
random.seed(42)
test_sample = random.sample(test_data, min(NUM_TEST_SAMPLES, len(test_data)))
...
test_texts = [INPUT_PREFIX + fix_bangla_for_tokenizer(item['text']) for item in test_sample]
test_summaries = [fix_bangla_for_tokenizer(item['summary']) for item in test_sample]
